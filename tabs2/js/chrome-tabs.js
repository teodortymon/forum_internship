(function() {
  var $, chromeTabs, defaultNewTabData, tabTemplate, tabArray=[];
  $ = jQuery;
...
//      });
    ...
    addNewTab: function($shell, setActive, newTabData) {
      var $newTab, tabData;
      $newTab = $(tabTemplate);
    ...
    setCurrentTab: function($shell, $tab) {
      ...
      sessionStorage.setItem('tabs',JSON.stringify(tabArray));                          //dodawanie tablicy kart do sessionStorage
      window.location.href = $shell.find('.chrome-tab-current').find('a').attr('href');     //podążanie za linkiem z wybranej karty
      return chromeTabs.render($shell);
    },
...
//      }
    ...
    updateTab: function($shell, $tab, tabData) {
      ...
      $tab.find('a').attr("href",tabData.href);
